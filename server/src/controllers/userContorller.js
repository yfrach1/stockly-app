...
async function createUser(req, res) {
   const { accessToken, user, portfolio } = await userService.createUser(req.body);
   accessToken
      ? res.status(200).cookie("token", accessToken, { httpOnly: true }).json({ user, portfolio })
...
async function loginUser(req, res) {
   ...
   accessToken
      ? res.status(200).cookie("token", accessToken, { httpOnly: true }).json(allUserData)
